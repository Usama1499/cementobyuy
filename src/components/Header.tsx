    ...
    <header className="sticky top-0 z-40 border-b border-border bg-background/85 backdrop-blur-md">
      <div className="container-page flex h-16 items-center justify-between gap-4 lg:h-20">
        <Link to="/" className="flex items-baseline gap-2" onClick={() => setOpen(false)}>
          <span className="font-display text-xl font-bold uppercase tracking-[0.32em] text-foreground">
            Cemento
          </span>
        </Link>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Link
            ...
            className="relative flex h-10 w-10 items-center justify-center rounded-sm text-foreground transition-colors hover:bg-secondary"
          >
            <ShoppingBag className="h-5 w-5" />
            {hydrated && count > 0 && (
              <span className="absolute right-1 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-clay px-1 text-[0.625rem] font-semibold text-clay-foreground">
                {count}
              </span>
            )}
          </Link>
        ...
        <div className="border-t border-border bg-background lg:hidden">
          <nav className="container-page flex flex-col py-2">
            ...
            <Link
              to="/contact"
              onClick={() => setOpen(false)}
              className="py-3 text-sm font-medium text-clay"
            >
              Get a quote
            </Link>
          </nav>
